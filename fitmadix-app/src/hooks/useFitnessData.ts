...
import { 
  fetchRoutines, saveRoutineToDb, deleteRoutineFromDb,
  fetchWorkoutLogs, saveWorkoutSession,
  fetchMeals, saveMealToDb, deleteMealFromDb,
  fetchExercises, fetchWeightLogs, saveWeightLogToDb,
  fetchUserSettings, saveUserSettingsToDb,
  fetchStrengthRecords, upsertStrengthRecord
} from "@/services/fitnessService";
import type { Routine, WorkoutSessionLog, MealItem, MacroTargets, UserMetrics, WeightLogEntry, StrengthRecord } from "@/types/fitness";
...
export function useFitnessData() {
        ...
        ex.sets.filter(s => s.completed && s.weightKg > 0 && s.reps > 0).forEach(set => {
          ...
          if (!existing || estimated1RM > existing.estimatedOneRepMaxKg) {
            upsertStrengthRecord({
              id: existing?.id || `sr-${Date.now()}-${Math.random().toString(36).substring(2, 5)}`,
  ...
  return {
    ...
    userMetrics,
    strengthRecords,
    isLoading: isLoadingRoutines || isLoadingLogs || isLoadingMeals || isLoadingExercises || isLoadingWeights || isLoadingSettings || isLoadingStrength,
