...
import { AIWorkoutPlan, AIMealPlan, Routine, RoutineExercise, UserMetrics, VitalsData } from '../../types/fitness';
...
export const AICoach: React.FC<AICoachProps> = ({ onSaveGeneratedRoutine, userMetrics, vitals }) => {
  ...
  const handleGenerateWorkout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to generate workout. Please try again.');
    } finally {
  ...
  const handleGenerateMealPlan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to generate meal plan. Please try again.');
    } finally {
  ...
  const handleSendChatMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { sender: 'coach', text: 'Sorry, I ran into an issue connecting. Please try again.' }]);
    } finally {
    ...
    <div className="flex-1 p-6 lg:p-8 space-y-6 overflow-y-auto">
        ...
        <div className="flex p-1 bg-zinc-900 border border-zinc-800 rounded-xl text-xs font-bold">
          <button
            ...
            className={`px-4 py-2 rounded-lg cursor-pointer flex items-center gap-2 transition-all ${
              activeSubTab === 'workout' ? 'bg-white text-black shadow' : 'text-zinc-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg cursor-pointer flex items-center gap-2 transition-all ${
              activeSubTab === 'meal' ? 'bg-white text-black shadow' : 'text-zinc-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg cursor-pointer flex items-center gap-2 transition-all ${
              activeSubTab === 'chat' ? 'bg-white text-black shadow' : 'text-zinc-400 hover:text-white'
            }`}
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 bg-zinc-950 border border-zinc-800 rounded-2xl p-6 shadow-2xl">
            ...
            <form onSubmit={handleGenerateWorkout} className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Primary Goal</label>
                <select
                  value={workoutGoal}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Experience</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Days / Week</label>
                  <select
                    value={daysPerWeek}
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Equipment</label>
                <input
                  type="text"
                  value={equipment}
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Target Focus</label>
                <input
                  type="text"
                  value={targetFocus}
            ...
            {generatedWorkout ? (
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  {generatedWorkout.exercises.map((ex, idx) => (
                    <div key={idx} className="p-3 bg-zinc-900 border border-zinc-800 rounded-xl flex justify-between items-start">
                      <div>
                        <p className="text-sm font-bold text-white">{ex.name}</p>
                        <p className="text-xs text-zinc-400 mt-0.5">{ex.reasoning}</p>
                      </div>
                      <div className="text-right shrink-0">
                        <span className="text-xs font-bold text-white block">{ex.sets} sets × {ex.reps}</span>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-12 text-zinc-500">
                ...
                <p className="text-xs max-w-sm mt-1">
                  Fill out your preferences on the left and click "Generate AI Routine" to get an evidence-based split.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-5 bg-zinc-950 border border-zinc-800 rounded-2xl p-6 shadow-2xl">
            ...
            <form onSubmit={handleGenerateMealPlan} className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Target Calories (kcal)</label>
                <input
                  type="number"
                  value={calorieTarget}
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Dietary Focus</label>
                <select
                  value={dietType}
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Allergies or Dislikes</label>
                <input
                  type="text"
            ...
            {generatedMealPlan ? (
              <div className="space-y-6">
                <div className="border-b border-zinc-800 pb-4">
                  <h2 className="text-2xl font-extrabold text-white tracking-tight">{generatedMealPlan.title}</h2>
                  ...
                  {generatedMealPlan.meals.map((m, idx) => (
                    <div key={idx} className="p-4 bg-zinc-900 border border-zinc-800 rounded-xl space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold uppercase text-blue-400">{m.timeSlot} • {m.mealName}</span>
                        <span className="text-xs font-bold text-white">{m.macros.calories} kcal</span>
                      </div>
                      <p className="text-xs text-zinc-300">{m.description}</p>
                      <div className="flex flex-wrap gap-1 pt-1">
                        {m.ingredients.map((ing, i) => (
                          <span key={i} className="text-[10px] px-2 py-0.5 bg-zinc-800 text-zinc-400 rounded">
                            {ing}
                          </span>
                        ))}
                      </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-12 text-zinc-500">
                ...
                <p className="text-xs max-w-sm mt-1">Enter your targets and click "Generate Meal Plan".</p>
              </div>
            )}
          </div>
        </div>
      ...
      {activeSubTab === 'chat' && (
        <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 shadow-2xl flex flex-col h-[520px]">
          ...
          {/* Input Box */}
          <form onSubmit={handleSendChatMessage} className="mt-4 pt-4 border-t border-zinc-800 flex gap-2">
            <input
              type="text"
