...
import { 
  LayoutDashboard, Dumbbell, BookOpen, Utensils, Sparkles, LineChart, Activity, User,
  MessageSquare, Camera, UtensilsCrossed, MapPin, Pill, Apple, Stethoscope,
  ChevronDown, ChevronRight, Home, ScanLine, ClipboardList
} from 'lucide-react';
...
export const SidebarRail: React.FC<SidebarRailProps> = ({
  ...
  const sections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'encyclopedia-disease', label: 'Diseases', icon: <Stethoscope className="w-5 h-5" /> },
      ],
    },
  ];
  ...
  const activeSection = sections.find(s => s.items.some(i => i.id === activeTab))?.title || 'Fitness';

  return (
    <aside
      className={`${isExpanded ? 'w-56' : 'w-20'} border-r border-zinc-800 flex flex-col py-6 gap-2 bg-zinc-950/80 backdrop-blur-md select-none shrink-0 z-20 transition-all duration-300`}
      onMouseEnter={() => setIsExpanded(true)}
      onMouseLeave={() => { setIsExpanded(false); setExpandedSection(null); }}
    >
      {/* Brand Logo */}
      <div className="flex items-center justify-center px-4 mb-4">
        <button
          onClick={() => onTabChange('dashboard')}
          className="w-10 h-10 bg-white rounded-xl flex items-center justify-center hover:scale-105 transition-transform cursor-pointer shadow-lg shadow-white/5 shrink-0"
          title="FitMadix Home"
        >
      ...
      <nav className="flex-1 flex flex-col gap-1 overflow-y-auto overflow-x-hidden px-2">
        {sections.map(section => {
          const isSectionExpanded = isExpanded && (expandedSection === section.title || activeSection === section.title);
            ...
            <div key={section.title}>
              ...
              {isExpanded && (
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer"
                >
              ...
              {/* Items */}
              <div className={`flex flex-col gap-0.5 ${isExpanded && !isSectionExpanded ? 'hidden' : ''}`}>
                {(isExpanded ? section.items : (sectionHasActive ? section.items.filter(i => i.id === activeTab) : section.items.slice(0, 1))).map(item => {
                  const isActive = activeTab === item.id;
                  return (
                    <button
                      key={item.id}
                      onClick={() => onTabChange(item.id)}
                      className={`group relative flex items-center gap-3 rounded-xl transition-all cursor-pointer ${
      ...
      <div className="px-2">
        <button
          ...
          className={`w-full flex items-center gap-3 rounded-xl border border-zinc-700 bg-gradient-to-tr from-zinc-800 to-zinc-950 hover:border-zinc-500 transition-colors text-zinc-300 hover:text-white cursor-pointer ${
            isExpanded ? 'px-3 py-2.5' : 'p-3 justify-center'
          }`}
