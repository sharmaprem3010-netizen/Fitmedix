...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  const weightChange = (weightLogs.length > 1 && initialWeight) 
  ...
  return (
    <div className="flex-1 p-6 lg:p-8 space-y-6 overflow-y-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-zinc-800 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Analytics & Progress</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <Card variant="dark">
          <p className="text-xs font-bold uppercase tracking-widest text-zinc-500">Current Weight</p>
          <p className="text-4xl font-extrabold text-white mt-1">{latestWeight} <span className="text-lg font-normal text-zinc-500">{weightLogs[0]?.weightKg ? 'kg' : ''}</span></p>
          <p className="text-xs text-emerald-400 mt-2">{weightLogs.length > 0 ? `${weightChange} kg change tracked` : 'No weight entries logged yet'}</p>
        </Card>

        <Card variant="dark">
          <p className="text-xs font-bold uppercase tracking-widest text-zinc-500">Est. Body Fat</p>
          <p className="text-4xl font-extrabold text-white mt-1">{latestBodyFat} <span className="text-lg font-normal text-zinc-500">{weightLogs[0]?.bodyFatPercent ? '%' : ''}</span></p>
          <p className="text-xs text-blue-400 mt-2">{weightLogs[0]?.bodyFatPercent ? 'Body composition recorded' : 'Log body fat % to track trend'}</p>
        </Card>

        <Card variant="dark">
          <p className="text-xs font-bold uppercase tracking-widest text-zinc-500">Completed Workouts</p>
          <p className="text-4xl font-extrabold text-white mt-1">{workoutLogs.length} <span className="text-lg font-normal text-zinc-500">sessions</span></p>
          <p className="text-xs text-amber-400 mt-2">{workoutLogs.length > 0 ? 'Live session logs active' : 'Complete a workout to log history'}</p>
        </Card>
      </div>
      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 shadow-2xl">
        ...
        {strengthRecords.length === 0 ? (
          <div className="p-8 text-center text-zinc-500 text-xs bg-zinc-900/50 rounded-xl border border-zinc-900">
            No 1-Rep Max records auto-calculated yet. Complete your first workout session to record strength PRs!
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {strengthRecords.map(sr => (
              <div key={sr.id} className="p-4 bg-zinc-900 border border-zinc-800 rounded-xl space-y-1">
                <span className="text-[10px] font-bold uppercase text-zinc-500">{sr.exerciseName}</span>
                <p className="text-2xl font-extrabold text-white">
                  {sr.estimatedOneRepMaxKg} <span className="text-xs font-normal text-zinc-400">kg 1RM</span>
                </p>
                <p className="text-xs text-zinc-400">Tested: {sr.weightKg}kg × {sr.reps} reps ({sr.date})</p>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="divide-y divide-zinc-900">
            {weightLogs.map(wl => (
              <div key={wl.id} className="px-6 py-3.5 flex justify-between items-center text-sm">
                ...
                <span className="text-zinc-400">{wl.bodyFatPercent ? `${wl.bodyFatPercent}% BF` : '--'}</span>
              </div>
            ))}
          </div>
        )}
      ...
      >
        <form onSubmit={handleSaveWeight} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold uppercase text-zinc-400 mb-1">Body Fat % (Optional)</label>
            <input
              type="number"
              step="0.1"
              value={newBodyFat}
