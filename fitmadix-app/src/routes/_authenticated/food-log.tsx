...
function FoodLogPage() {
  ...
  const { entries, summary, loading, addEntry, removeEntry, waterGlasses, updateWater } = useFoodLog(dateStr);
    ...
    <div className="min-h-dvh bg-background pb-20">
      <div className="mx-auto max-w-2xl px-4 py-6 sm:px-6">
        ...
        <div className="mb-6 flex items-center justify-between rounded-full bg-secondary p-1">
          <button onClick={() => changeDate(-1)} className="grid h-10 w-10 place-items-center rounded-full hover:bg-background">
        ...
        <div className="mb-6 rounded-3xl border border-border bg-card p-6 shadow-elegant">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Eaten</p>
              <p className="text-2xl font-bold">{Math.round(summary.calories)}</p>
            </div>
            {/* Simple CSS ring chart */}
            <div className="relative h-24 w-24">
              <svg className="h-full w-full" viewBox="0 0 36 36">
                <path className="text-secondary stroke-current" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" strokeWidth="4" />
                <path className="text-primary stroke-current transition-all duration-1000" strokeDasharray={`${calProgress}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" strokeWidth="4" strokeLinecap="round" />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-lg font-bold">{Math.max(0, goals.calories - Math.round(summary.calories))}</span>
          ...
          <div className="grid grid-cols-3 gap-4 border-t border-border/50 pt-4">
            <div>
              ...
              <div className="mt-1 h-1.5 w-full rounded-full bg-secondary overflow-hidden">
                <div className="h-full bg-blue-500" style={{ width: `${Math.min(summary.protein / goals.protein * 100, 100)}%` }} />
              </div>
              <p className="mt-1 text-[10px]">{Math.round(summary.protein)} / {goals.protein}g</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Carbs</p>
              <div className="mt-1 h-1.5 w-full rounded-full bg-secondary overflow-hidden">
                <div className="h-full bg-amber-500" style={{ width: `${Math.min(summary.carbs / goals.carbs * 100, 100)}%` }} />
              </div>
              <p className="mt-1 text-[10px]">{Math.round(summary.carbs)} / {goals.carbs}g</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Fat</p>
              <div className="mt-1 h-1.5 w-full rounded-full bg-secondary overflow-hidden">
                <div className="h-full bg-rose-500" style={{ width: `${Math.min(summary.fat / goals.fat * 100, 100)}%` }} />
              </div>
              <p className="mt-1 text-[10px]">{Math.round(summary.fat)} / {goals.fat}g</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {meals.map((meal) => {
            ...
            return (
              <div key={meal.type} className="rounded-2xl border border-border bg-card p-5 shadow-soft">
                ...
                {mealEntries.length > 0 ? (
                  <div className="mb-4 space-y-3">
                    {mealEntries.map((e) => (
                      <div key={e.id} className="flex items-start justify-between border-b border-border/50 pb-2 last:border-0 last:pb-0">
                        <div>
                          <p className="text-sm font-medium">{e.food_name}</p>
                          <p className="text-[10px] text-muted-foreground">
                            P: {Math.round(e.protein_g)}g • C: {Math.round(e.carbs_g)}g • F: {Math.round(e.fat_g)}g
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-bold">{Math.round(e.calories)}</span>
                          <button onClick={() => removeEntry(e.id)} className="text-red-500 hover:opacity-70 text-xs">❌</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <button
                  onClick={() => { setActiveMeal(meal.type); setIsAddModalOpen(true); }}
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-5 shadow-soft">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm font-medium text-muted-foreground">{waterGlasses} / {goals.water} glasses</span>
          </div>
            ...
            {Array.from({ length: Math.max(goals.water, waterGlasses + 1) }).map((_, i) => (
              <button
                ...
                className={`grid h-10 w-10 place-items-center rounded-xl transition-all ${
                  i < waterGlasses ? "bg-cyan-500 text-white" : "bg-secondary text-muted-foreground hover:bg-cyan-100"
                }`}
              >
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/50 backdrop-blur-sm sm:items-center sm:justify-center">
          <div className="w-full rounded-t-3xl bg-background p-6 shadow-2xl sm:max-w-md sm:rounded-3xl animate-in slide-in-from-bottom-10">
            <div className="mb-6 flex items-center justify-between">
              ...
              <button onClick={() => setIsAddModalOpen(false)} className="rounded-full p-2 hover:bg-secondary">❌</button>
            </div>
            ...
            <p className="mt-4 text-center text-xs text-muted-foreground">
              Just type what you ate naturally (e.g. "2 slices of bread with peanut butter"). AI will estimate the calories and macros.
            </p>
          </div>
        </div>
      )}
